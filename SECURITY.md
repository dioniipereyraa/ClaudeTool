...
| Email de la cuenta en la cabecera del `.md` | El `.md` se pega en Claude Code o se comparte con el email adentro | Opt-in en todas las capas (`exportal.includeAccountEmail`, apagado por defecto; `--include-account-email` en el CLI). El Companion solo consulta el endpoint de cuenta del propio sitio (`/api/account`, `/api/auth/session`) cuando el bridge lo pide en `/ping`, y el bridge solo lo escribe bajo el mismo toggle. `--redact-pii` lo enmascara igual. Viaja solo por el bridge local |
