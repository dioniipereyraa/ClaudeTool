...
- Un solo elemento memorable: siete hilos en SVG detrás del hero, de la izquierda (chat) a la derecha (editor), que se dibujan al cargar y paquetes con estela que viajan por ellos. Sin JS quedan los hilos quietos; con `prefers-reduced-motion` no hay paquetes ni draw-in.
  - **Primera versión, rota en el navegador de Dionisio:** el draw-in usaba `stroke-dasharray: 1` normalizado con `pathLength="1"` más `vector-effect: non-scaling-stroke`, y los paquetes usaban CSS `offset-path` sobre `<circle>`. Safari mide el dash en píxeles de pantalla cuando el trazo es non-scaling (hilos a medio dibujar) y no todos los motores aplican `offset-path` a elementos SVG (cero paquetes). Se reescribió con mecanismos universales: dasharray a partir de `getTotalLength()` con una transición CSS, y `<animateMotion>` + `<mpath>` nativos del SVG para los paquetes. El fondo pasó de `preserveAspectRatio="none"` a `xMidYMid slice`, así los hilos cruzan siempre de borde a borde sin deformar el trazo.
